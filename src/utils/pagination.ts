 ...
 */
export const generateUrl = (url: URL | string, page?: string) => {
...
export const paginate = async <T>(data: T[], url: URL) => {
	...
	const last = pageNum === lastPage ? undefined : generateUrl(url, String(lastPage))
	const prev =
		pageNum === 1
			? undefined
			: generateUrl(url, pageNum - 1 === 1 ? undefined : String(pageNum - 1))
	const first = pageNum === 1 ? undefined : generateUrl(url)
	return {
		...
		currentPage: pageNum,
		lastPage: lastPage,
		url: { current, prev, next, first, last }
	} as Page<T>
}
